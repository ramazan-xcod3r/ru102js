...
const findAll = async () => {
  ...
  for (const siteKey of siteKeys) {
    siteHashes.push(await client.hgetallAsync(siteKey))
  }
  siteHashes = siteHashes.map((siteHash) => {
    if (siteHash !== null) return siteHash;
  });
  return siteHashes.map((siteHash) =>  remap(siteHash));
};
/* eslint-enable */
